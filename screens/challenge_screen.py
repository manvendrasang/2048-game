# pylint: disable=missing-module-docstring, missing-function-docstring, no-member, f-string-without-interpolation
...
class ChallengeScreen:
    """
    Returns:
        int    — challenge id to start (after preview confirmed)
        "back" — return to menu
        None   — no action
    """
    ...
    def __init__(self, surface: pygame.Surface):
        ...
        from screens.challenge_preview import ChallengePreview
        self._preview = ChallengePreview(surface)
    ...
    def handle_event(self, event) -> int | str | None:
        # Let preview handle events first when visible
        if self._preview.is_visible():
            result = self._preview.handle_event(event)
            if result is None:
                return None
            if result == "cancel":
                return None
            if isinstance(result, tuple) and result[0] == "start":
                return result[1]   # challenge id → starts game
            return None
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
            ...
            for i, ch in enumerate(challenges):
                if self._card_rect(i).collidepoint(px, py):
                    ...
                    self._preview.open(ch, progress)
                    return None   # don't start yet — wait for preview
        return None
    ...
    def draw(self):
        ...
        # Draw preview modal on top if open
        self._preview.draw()
