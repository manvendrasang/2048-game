# pylint: disable=missing-module-docstring, missing-function-docstring, missing-class-docstring, no-member, line-too-long
...
from data.persistence import load_leaderboard_by_mode, load_daily_leaderboard
...
class LeaderboardScreen:
    ...
    def draw(self, _=None):
        ...
        if mode_filter == MODE_DAILY:
            entries = load_daily_leaderboard()
            from data.daily_puzzle import seconds_until_next_puzzle, daily_date_str, get_daily_description
            secs = seconds_until_next_puzzle()
            hh   = secs // 3600
            mm   = (secs % 3600) // 60
            ss   = secs % 60
            reset_txt = font("hint").render(
                f"Today: {daily_date_str()}   Resets in {hh:02d}:{mm:02d}:{ss:02d}",
                True, th["hint_text"],
            )
            blit_centered(srf, reset_txt, WIN_W // 2, CONTENT_TOP - 24)
            desc_txt = font("hint").render(
                get_daily_description(), True, th["lbl_text"]
            )
            blit_centered(srf, desc_txt, WIN_W // 2, CONTENT_TOP - 8)
        else:
