...
DEFAULT_SCORE = 2
pygame.init()
SURFACE = pygame.display.set_mode((600, 500), 0, 32)
BLACK = (0, 0, 0)
...
def canMove():
    for i in range(0, BOARD_SIZE):
        for j in range(1, BOARD_SIZE):
            ...
            elif (tileMatrix[i][j - 1] == tileMatrix[i][j]) and tileMatrix[i][
                j - 1
            ] != 0:
                return True
...
def printMatrix():
    ...
    for i in range(0, BOARD_SIZE):
        for j in range(0, BOARD_SIZE):
            ...
            label2 = scorefont.render(
                "YourScore:" + str(TOTAL_POINTS), 1, (255, 255, 255)
            )
            SURFACE.blit(
                label, (i * (400 / BOARD_SIZE) + 30, j * (400 / BOARD_SIZE) + 130)
            )
...
def checkIfCanGo():
    for i in range(
        0, BOARD_SIZE**2
    ):  # This was having an error as that is 4 power of 2
...
def saveGameState():
    ...
    line1 = " ".join(
        [
            str(tileMatrix[floor(x / BOARD_SIZE)][x % BOARD_SIZE])
